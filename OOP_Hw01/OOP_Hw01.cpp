...
public:
	NODE();											//建構子
	unsigned int  SendTo(unsigned int Dst,unsigned int NodeMinus);			
...
//這個node和那些node是鄰居
void NODE::StoreNeighbor(unsigned int NeighborNode) {					
...
//得到鄰居有哪些
vector<unsigned int>* NODE::GetNeighbor() {						
	vector <unsigned int >* VectorOut = new vector<unsigned int>;
	*VectorOut = Neighbor;
	return VectorOut;
}

unsigned int  NODE::SendTo(unsigned int Dst,unsigned int NodeMinus) {			
...
//建routing table, |終點|下個節點|2欄
void NODE::MakeARoutingTable(NODE* ptr,unsigned int Node) {					
...
int main() {
	...
	NODE* ptr = NULL;
	unsigned int NodeNumber = 0, PathNumber = 0;
...
//create the object NumberOfNode
NODE*  CreateObject(unsigned int NumOfNode) {						 
	NODE *PTR = new NODE[NumOfNode];						
